...
import { ArrowLeft, Loader2, Phone, MapPin, FileText, CreditCard, TrendingUp, ArrowDownLeft, ArrowUpRight } from 'lucide-react';
...
export function CustomerDetailPage() {
  ...
  const generatePDFDoc = async () => {
    ...
    doc.text(`Mobile: ${customer.mobile || 'N/A'}`, 14, 35);
    doc.text(`GSTIN: ${customer.gst_number || 'N/A'}`, 14, 40);
    ...
    doc.setFontSize(8);
    doc.setTextColor(115, 115, 115);
    doc.text('Total Invoiced', 115, 58);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(11);
    doc.setTextColor(29, 78, 216);
    doc.text(formatCurrencyPDF(totalInvoiced), 115, 65);
    doc.setFont('helvetica', 'italic');
    doc.setFontSize(7);
    doc.setTextColor(163, 163, 163);
    doc.text(`${invoices.length} invoice(s)`, 115, 70);

    // Card 3: Total Received (Emerald)
    ...
    doc.setFontSize(8);
    doc.setTextColor(115, 115, 115);
    doc.text('Total Received', 21, 84);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(11);
    doc.setTextColor(4, 120, 87);
    doc.text(formatCurrencyPDF(totalReceived), 21, 91);
    doc.setFont('helvetica', 'italic');
    doc.setFontSize(7);
    doc.setTextColor(163, 163, 163);
    ...
    const allTxns: any[] = [];
    invoices.forEach((inv: any) => allTxns.push({ ...inv, _type: 'invoice', _date: inv.invoice_date, _ref: inv.invoice_number, _amount: Number(inv.total_amount) || 0, _paid: Number(inv.paid_amount) || 0, _mode: null, _txnRef: null }));
    payments.forEach((p: any) => allTxns.push({ ...p, _type: 'payment', _date: p.payment_date, _ref: p.payment_no || ('#' + p.id), _amount: Number(p.amount) || 0, _paid: 0, _mode: p.payment_mode?.name, _txnRef: p.transaction_reference }));
    allTxns.sort((a: any, b: any) => new Date(b._date).getTime() - new Date(a._date).getTime());
    
    allTxns.forEach((txn: any) => {
      const isInvoice = txn._type === 'invoice';
      const dateStr = formatDate(txn._date);
      const typeStr = isInvoice ? 'Invoice' : 'Payment';
      const refStr = txn._ref;
      const detailsStr = isInvoice ? `${txn.items?.length || 0} items` : (txn._mode ? `Via ${txn._mode}` : '-');
      const amountStr = `${isInvoice ? '+' : '-'}${formatCurrencyPDF(txn._amount)}`;
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [pdfFile] })) {
        ...
        await navigator.share({
          ...
          text: `Financial summary statement for ${customer.name}.`
        });
      } else {
        ...
        const text = `*Customer Financial Summary:*
*Name:* ${customer.name}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-4 justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-500">Customer since {customer.created_at ? new Date(customer.created_at).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' }) : 'N/A'}</p>
          </div>
        </div>
      ...
      <div className="card rounded-2xl">
        ...
        {/* Merge invoices and payments into single timeline */}
        {(() => {
          const allTxns: any[] = [];
          invoices.forEach((inv: any) => allTxns.push({ ...inv, _type: 'invoice', _date: inv.invoice_date, _ref: inv.invoice_number, _amount: Number(inv.total_amount) || 0, _paid: Number(inv.paid_amount) || 0, _mode: null, _txnRef: null }));
          payments.forEach((p: any) => allTxns.push({ ...p, _type: 'payment', _date: p.payment_date, _ref: p.payment_no || ('#' + p.id), _amount: Number(p.amount) || 0, _paid: 0, _mode: p.payment_mode?.name, _txnRef: p.transaction_reference }));

          const filtered = txnFilter === 'all' ? allTxns : allTxns.filter((t: any) => t._type === (txnFilter === 'invoices' ? 'invoice' : 'payment'));
                  ...
                  {filtered.map((txn: any) => {
                    ...
                    return (
                      <tr
                        ...
                        className={'border-b border-neutral-50 transition-colors ' + (isInvoice ? 'cursor-pointer hover:bg-blue-50/50' : 'hover:bg-emerald-50/50')}
                      >
                        <td className="px-5 py-3">
                          <div className={'w-8 h-8 rounded-xl flex items-center justify-center ' + (isInvoice ? 'bg-blue-50' : 'bg-emerald-50')}>
                            {isInvoice ? (
                        ...
                        <td className="px-5 py-3">
                          <p className={'font-medium ' + (isInvoice ? 'text-neutral-900' : 'text-emerald-700')}>{txn._ref}</p>
                          <p className="text-xs text-neutral-400">{isInvoice ? 'Invoice' : 'Payment'}{txn._mode ? ' via ' + txn._mode : ''}</p>
                        </td>
                        ...
                        <td className="px-5 py-3 text-right font-mono font-semibold tabular-nums">
                          <span className={isInvoice ? 'text-blue-600' : 'text-emerald-600'}>
                            {isInvoice ? '+' : '-'}{formatCurrency(txn._amount)}
                          </span>
                        </td>
